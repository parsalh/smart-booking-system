    ...
    public ResponseEntity<?> suggestTimes(@RequestBody BookingRequest request,
        ...
        try {
            ...
            for (String email : allEmailsToFetch) {
                Optional<User> participantOpt = userRepository.findByEmail(email);
                if (participantOpt.isEmpty()) {
                    continue;
                }

                User participant = participantOpt.get();
                registeredEmailsToFetch.add(email);
            ...
            Map<String, List<TimePeriod>> busyBlocks =
                    availabilityService.fetchGroupAvailability(registeredEmailsToFetch, searchStart, searchEnd, organizer);
